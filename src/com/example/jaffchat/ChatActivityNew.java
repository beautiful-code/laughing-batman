...
public class ChatActivityNew extends Activity {
	...
	BroadcastReceiver notificationReceiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
			...
			Map map = ChatActivityNew.createRoomMap(email, message, clipUrl,time);
			list.add(map);
	...
	BroadcastReceiver initializationReceiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
			Log.d(TAG, "received broadcast to initialize chat screen messages ..going to notify adapter");
		  //list =  (List<Map>) intent.getSerializableExtra("messages");			
			...
			Handler handler = new Handler();    //This is to delay the scroll till after the web images load..else it wont scroll to the bottom...bcos it scrolls before image gets expanded
...
//		RelativeLayout.LayoutParams params1 = (RelativeLayout.LayoutParams) w1.getLayoutParams();   //190 , 130   width,height
//		RelativeLayout.LayoutParams params2 = (RelativeLayout.LayoutParams) w1.getLayoutParams();   //190 , 130   width,height
//		LinearLayout.LayoutParams params3 = (LinearLayout.LayoutParams) listView.getLayoutParams();   //190 , 130   width,height
//
//		params1.height = 1;
//		params2.height = 1;
//		params1.width = 1;
...
//        listView.setLayoutParams(params3);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Intent intent1 = new Intent("com.example.locationbased.action.getmessages");
		...
		getImages.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Intent intent = new Intent(
						"com.example.locationbased.action.getimages");   //I added postmessage just to differentiate from the old sendmessage which inserts messages into the db...since i dont wanna do that right now , i'm using a separate service.
...
//		        listView.setLayoutParams(params3);
		...
		sendMessage.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				String messageValue = message.getText().toString();
				Intent intent = new Intent(
						"com.example.locationbased.action.postmessage");   //I added postmessage just to differentiate from the old sendmessage which inserts messages into the db...since i dont wanna do that right now , i'm using a separate service.
				intent.putExtra("message", messageValue);
				intent.putExtra("message_url",
						"http://imageshack.us/a/img706/8427/brahmi.gif");  //TODO This is actually not relevant...because we are sending a predefined clip id ...and not a url....Change this when u add a search endpoint ot add gif dynamically
	...
	public static Map<String, String> createRoomMap(String email,String message,String clipUrl,String time) {
		Map map;
		map = new HashMap();
		map.put("email", email);
	...
	public class GetMessages extends AsyncTask<String, String, Boolean> {
		...
		protected Boolean doInBackground(String... params) {
			...
			HttpGet httpGet = new HttpGet(UserData.HOST+"/rooms/"+roomId+"/messages.json?"+"auth_token="+authentication_token+"&since="+timeOfMostRecentMessage); 
			...
			try {
				...
				Log.d(TAG, "Response code is"+new Integer(responseCode).toString());
				...
				case 200:
					...
					for (int i = 0; i < messagesArray.length(); i++) {
						Log.d(TAG, "Response is ChatActivity Polling TASK is iterating for "+i+"th time");
