...
public class GetImagesService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		// String full_url =
		// UserData.HOST+"/search.json?"+"auth_token="+authentication_token+"&q="+message;
		Uri.Builder builder = new Uri.Builder();
		builder.scheme("http").authority("54.254.137.190")
				.appendPath("search.json")
				.appendQueryParameter("auth_token", authentication_token)
		...
		try {
			...
			case 200:
				...
				for (int i = 0; i < imagesArray.length(); i++) {
					...
					ChatActivityNew.addToImageList(ChatActivityNew
							.createImageMap(clipId, file_path));
				}
				...
				if (intent.getStringExtra("source").equals("image_polling")) {
